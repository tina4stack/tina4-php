/**
 * Tina4 Dev Admin — vanilla JS, zero dependencies, cross-language.
 *
 * This file is shared across all Tina4 frameworks (Python, PHP, Ruby, Node.js).
 * Keep it framework-agnostic — no Python/PHP/Ruby/Node.js specific code.
 */
...
function loadTables() {
    api('/__dev/api/tables').then(d => {
        ...
        document.getElementById('table-list').innerHTML = tables.map(t =>
            `<div style="padding:0.2rem 0.4rem;cursor:pointer;border-radius:0.25rem" onclick="browseTable('${t}')" onmouseover="this.style.background='rgba(123,31,162,0.1)'" onmouseout="this.style.background=''">${t}</div>`
        ).join('');
...
function loadSystem() {
    api('/__dev/api/system').then(d => {
        // Handle both Python and PHP system API responses
...
// Ctrl+Enter for database queries
