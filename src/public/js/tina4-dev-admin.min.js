...
`,we="/__dev/api";async function B(t,e="GET",n){const i={method:e,headers:{}};return n&&(i.headers["Content-Type"]="application/json",i.body=JSON.stringify(n)),(await fetch(we+t,i)).json()}function a(t){const e=document.createElement("span");return e.textContent=t,e.innerHTML}function $e(t){t.innerHTML=`
  ...
  `).join(""))}window.__loadRoutes=Nt;let W=[],U=[],H=JSON.parse(localStorage.getItem("tina4_query_history")||"[]");function ke(t){t.innerHTML=`
    <div class="dev-panel-header">
      <h2>Database</h2>
  ...
  `,vt(),wt()}async function vt(){const e=(await B("/tables")).tables||[],n=document.getElementById("db-table-list");n&&(n.innerHTML=e.length?e.map(r=>`<div style="padding:0.3rem 0.5rem;cursor:pointer;border-radius:0.25rem;font-size:0.8rem;font-family:monospace" class="db-table-item" onclick="window.__selectTable('${a(r)}')" onmouseover="this.style.background='var(--border)'" onmouseout="this.style.background=''">${a(r)}</div>`).join(""):'<div class="text-sm text-muted">No tables</div>');const i=document.getElementById("db-seed-table");i&&(i.innerHTML='<option value="">Pick table...</option>'+e.map(r=>`<option value="${a(r)}">${a(r)}</option>`).join(""));const o=document.getElementById("paste-table");o&&(o.innerHTML='<option value="">Select table...</option>'+e.map(r=>`<option value="${a(r)}">${a(r)}</option>`).join(""))}function xt(t){var n;(n=document.getElementById("db-limit"))!=null&&n.value;const e=document.getElementById("db-query");e&&(e.value=`SELECT * FROM ${t}`),document.querySelectorAll(".db-table-item").forEach(i=>{i.style.background=i.textContent===t?"var(--border)":""}),Dt()}function _e(){var n;const t=document.getElementById("db-query"),e=((n=document.getElementById("db-limit"))==null?void 0:n.value)||"20";t!=null&&t.value&&(t.value=t.value.replace(/LIMIT\s+\d+/i,`LIMIT ${e}`))}function Ee(t){const e=t.trim();e&&(H=H.filter(n=>n!==e),H.unshift(e),H.length>50&&(H=H.slice(0,50)),localStorage.setItem("tina4_query_history",JSON.stringify(H)),wt())}function wt(){const t=document.getElementById("db-history");t&&(t.innerHTML='<option value="">Query history...</option>'+H.map((e,n)=>`<option value="${n}">${a(e.length>80?e.substring(0,80)+"...":e)}</option>`).join(""))}function Se(t){const e=parseInt(t);if(isNaN(e)||!H[e])return;const n=document.getElementById("db-query");n&&(n.value=H[e]),document.getElementById("db-history").selectedIndex=0}function Te(){H=[],localStorage.removeItem("tina4_query_history"),wt()}async function Dt(){var o,r,m;const t=document.getElementById("db-query"),e=(o=t==null?void 0:t.value)==null?void 0:o.trim();if(!e)return;Ee(e);const n=document.getElementById("db-result"),i=((r=document.getElementById("db-type"))==null?void 0:r.value)||"sql";n&&(n.innerHTML='<p class="text-muted">Running...</p>');try{const d=parseInt(((m=document.getElementById("db-limit"))==null?void 0:m.value)||"20"),u=await B("/query","POST",{query:e,type:i,limit:d});if(u.error){n&&(n.innerHTML=`<p style="color:var(--danger)">${a(u.error)}</p>`);return}u.rows&&u.rows.length>0?(U=Object.keys(u.rows[0]),W=u.rows,n&&(n.innerHTML=`<p class="text-sm text-muted" style="margin-bottom:0.5rem">${u.count??u.rows.length} rows</p>
        <div style="overflow-x:auto"><table><thead><tr>${U.map(b=>`<th>${a(b)}</th>`).join("")}</tr></thead>
        <tbody>${u.rows.map(b=>`<tr>${U.map(c=>`<td class="text-sm">${a(String(b[c]??""))}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`)):u.affected!==void 0?(n&&(n.innerHTML=`<p class="text-muted">${u.affected} rows affected. ${u.success?"Success.":""}</p>`),W=[],U=[]):(n&&(n.innerHTML='<p class="text-muted">No results</p>'),W=[],U=[])}catch(d){n&&(n.innerHTML=`<p style="color:var(--danger)">${a(d.message)}</p>`)}}function Ie(){if(!W.length)return;const t=U.join(","),e=W.map(n=>U.map(i=>{const o=String(n[i]??"");return o.includes(",")||o.includes('"')?`"${o.replace(/"/g,'""')}"`:o}).join(","));navigator.clipboard.writeText([t,...e].join(`
`))}function Me(){W.length&&navigator.clipboard.writeText(JSON.stringify(W,null,2))}function Le(){const t=document.getElementById("db-paste-modal");t&&(t.style.display="flex")}function Ft(){const t=document.getElementById("db-paste-modal");t&&(t.style.display="none")}async function Ce(){var o,r,m,d,u;const t=(o=document.getElementById("paste-table"))==null?void 0:o.value,e=(m=(r=document.getElementById("paste-new-table"))==null?void 0:r.value)==null?void 0:m.trim(),n=e||t,i=(u=(d=document.getElementById("paste-data"))==null?void 0:d.value)==null?void 0:u.trim();if(!n||!i){alert("Select a table or enter a new table name, and paste data.");return}try{let b;try{b=JSON.parse(i),Array.isArray(b)||(b=[b])}catch{const $=i.split(`
`).map(k=>k.trim()).filter(Boolean);if($.length<2){alert("CSV needs at least a header row and one data row.");return}const h=$[0].split(",").map(k=>k.trim().replace(/[^a-zA-Z0-9_]/g,""));b=$.slice(1).map(k=>{const S=k.split(",").map(C=>C.trim()),M={};return h.forEach((C,Y)=>{M[C]=S[Y]??""}),M})}if(!b.length){alert("No data rows found.");return}if(e){const h=["id INTEGER PRIMARY KEY AUTOINCREMENT",...Object.keys(b[0]).filter(S=>S.toLowerCase()!=="id").map(S=>`"${S}" TEXT`)],k=await B("/query","POST",{query:`CREATE TABLE IF NOT EXISTS "${e}" (${h.join(", ")})`,type:"sql"});if(k.error){alert("Create table failed: "+k.error);return}}let c=0;for(const $ of b){const h=e?Object.keys($).filter(C=>C.toLowerCase()!=="id"):Object.keys($),k=h.map(C=>`"${C}"`).join(","),S=h.map(C=>`'${String($[C]).replace(/'/g,"''")}'`).join(","),M=await B("/query","POST",{query:`INSERT INTO "${n}" (${k}) VALUES (${S})`,type:"sql"});if(M.error){alert(`Row ${c+1} failed: ${M.error}`);break}c++}document.getElementById("paste-data").value="",document.getElementById("paste-new-table").value="",document.getElementById("paste-table").selectedIndex=0,Ft(),vt(),c>0&&xt(n)}catch(b){alert("Import error: "+b.message)}}async function Be(){var n,i;const t=(n=document.getElementById("db-seed-table"))==null?void 0:n.value,e=parseInt(((i=document.getElementById("db-seed-count"))==null?void 0:i.value)||"10");if(t)try{const o=await B("/seed","POST",{table:t,count:e});o.error?alert(o.error):xt(t)}catch(o){alert("Seed error: "+o.message)}}window.__loadTables=vt,window.__selectTable=xt,window.__updateLimit=_e,window.__runQuery=Dt,window.__copyCSV=Ie,window.__copyJSON=Me,window.__showPaste=Le,window.__hidePaste=Ft,window.__doPaste=Ce,window.__seedTable=Be,window.__loadHistory=Se,window.__clearHistory=Te;function ze(t){t.innerHTML=`
    <div class="dev-panel-header">
...
${n.traceback||""}`;window.__switchTab("chat"),setTimeout(()=>{window.__prefillChat(m)},150)}window.__loadErrors=lt,window.__clearErrors=je,window.__resolveError=Ae,window.__askAboutError=He;function qe(t){t.innerHTML=`
    <div class="dev-panel-header">
      <h2>System</h2>
    </div>
  ...
  `).join(""),n){const d=[];t.debug!==void 0&&d.push(["TINA4_DEBUG",String(t.debug)]),t.log_level&&d.push(["LOG_LEVEL",t.log_level]),t.database&&d.push(["DATABASE_URL",t.database]),d.length&&(n.innerHTML=`
        ...
        <table>
          ...
          <tbody>${d.map(([u,b])=>`<tr><td class="text-mono text-sm" style="padding:4px 8px">${a(u)}</td><td class="text-sm" style="padding:4px 8px">${a(b)}</td></tr>`).join("")}</tbody>
        </table>
  ...
  `,Wt()}async function Ne(){const t=await B("/metrics"),e=document.getElementById("metrics-quick");!e||t.error||(e.innerHTML=[E("Files",t.file_count),E("Lines of Code",t.total_loc),E("Blank Lines",t.total_blank),E("Comments",t.total_comment),E("Classes",t.classes),E("Functions",t.functions),E("Routes",t.route_count),E("ORM Models",t.orm_count),E("Templates",t.template_count),E("Migrations",t.migration_count),E("Avg File Size",(t.avg_file_size??0)+" LOC")].join(""))}async function Wt(){var r;const t=document.getElementById("metrics-chart"),e=document.getElementById("metrics-complex"),n=document.getElementById("metrics-scan-info");t&&(t.style.display="block",t.innerHTML='<p class="text-muted">Analyzing...</p>');const i=await B("/metrics/full");if(i.error||!i.file_metrics){t&&(t.innerHTML=`<p style="color:var(--danger)">${a(i.error||"No data")}</p>`);return}n&&(n.textContent=`${i.files_analyzed} files analyzed | ${i.total_functions} functions | Mode: ${i.scan_mode||"project"}`);const o=document.getElementById("metrics-quick");o&&(o.innerHTML=[E("Files Analyzed",i.files_analyzed),E("Total Functions",i.total_functions),E("Avg Complexity",i.avg_complexity),E("Avg Maintainability",i.avg_maintainability),E("Scan Mode",i.scan_mode||"project")].join("")),t&&i.file_metrics.length>0?De(i.file_metrics,t,i.dependency_graph||{}):t&&(t.innerHTML='<p class="text-muted">No files to visualize</p>'),e&&((r=i.most_complex_functions)!=null&&r.length)&&(e.innerHTML=`
    ...
    `)}function De(t,e,n){var ue,pe,ge,be,he,ye;e.clientWidth;const i=450,o=Math.max(...t.map(s=>s.loc||1)),r=18,m=50,d=1e3,u=1e3,c=[...t].sort((s,l)=>{const p=(s.avg_complexity??0)*2+(s.loc||0);return(l.avg_complexity??0)*2+(l.loc||0)-p}).map(s=>({...s,r:Math.max(r,Math.min(m,Math.sqrt((s.loc||1)/o)*m)),x:d,y:u}));for(let s=0;s<c.length;s++){if(s===0)continue;let l=0,p=0,y=!1;for(;!y;){const g=d+Math.cos(l)*p,f=u+Math.sin(l)*p;let x=!1;for(let v=0;v<s;v++){const _=g-c[v].x,A=f-c[v].y;if(Math.sqrt(_*_+A*A)<c[s].r+c[v].r+4){x=!0;break}}x||(c[s].x=g,c[s].y=f,y=!0),l+=.3,p+=.5}}for(const s of c)s.x+=(s.x-d)*1.5,s.y+=(s.y-u)*1.5;let $=1/0,h=-1/0,k=1/0,S=-1/0;for(const s of c)$=Math.min($,s.x-s.r-15),h=Math.max(h,s.x+s.r+15),k=Math.min(k,s.y-s.r-15),S=Math.max(S,s.y+s.r+25);const M=30,C=$-M,Y=k-M,J=h-$+M*2,X=S-k+M*2,R=Math.max(20,Math.round(Math.max(J,X)/20));e.innerHTML=`
  ...
  </defs>`;const K={};for(const[s,l]of Object.entries(n))if(ht[s])for(const p of l)K[p]||(K[p]=[]),K[p].includes(s)||K[p].push(s);const ae=new Set;L+='<g class="dep-lines">';for(const s of Object.values(K))if(!(s.length<2))for(let l=0;l<s.length;l++)for(let p=l+1;p<s.length;p++){const y=[s[l],s[p]].sort().join("|");if(ae.has(y))continue;ae.add(y);const g=ht[s[l]],f=ht[s[p]];if(!g||!f)continue;const x=f.x-g.x,v=f.y-g.y,_=Math.sqrt(x*x+v*v)||1,A=x/_,j=v/_,N=g.x+A*(g.r+2),ft=g.y+j*(g.r+2),P=f.x-A*(f.r+2),at=f.y-j*(f.r+2);L+=`<line x1="${N}" y1="${ft}" x2="${P}" y2="${at}" stroke="var(--info)" stroke-width="1.5" stroke-opacity="0.4" marker-end="url(#dep-arrow)" />`}L+="</g>";for(const s of c){const l=s.maintainability??50,p=s.has_tests?15:-15,y=Math.min((s.dep_count??0)*3,20),x=`hsl(${Math.min(120,Math.max(0,l*1.2+p-y))}, 80%, 45%)`,v=((ue=s.path)==null?void 0:ue.split("/").pop())||"?",_=s.has_tests===!0,A=s.dep_count??0;if(L+=`<circle cx="${s.x}" cy="${s.y}" r="${s.r}" fill="${x}" fill-opacity="0.6" stroke="${x}" stroke-width="1.5" style="cursor:pointer" data-drill="${a(s.path)}" />`,L+=`<title>${a(s.path)}
LOC: ${s.loc} | CC: ${s.avg_complexity} | MI: ${l}${_?" | Tested":""}${A>0?" | Deps: "+A:""}</title>`,s.r>15){const j=v.length>12?v.substring(0,10)+"..":v;L+=`<text x="${s.x}" y="${s.y+2}" text-anchor="middle" fill="white" font-size="8" font-weight="600" style="pointer-events:none" data-for="${a(s.path)}" data-role="label">${a(j)}</text>`}if(_){const j=s.x,N=s.y+s.r-10;L+=`<circle cx="${j}" cy="${N}" r="7" fill="var(--success)" stroke="var(--surface)" stroke-width="1" data-for="${a(s.path)}" data-role="t-circle" />`,L+=`<text x="${j}" y="${N+3}" text-anchor="middle" fill="white" font-size="7" font-weight="700" style="pointer-events:none" data-for="${a(s.path)}" data-role="t-text">T</text>`}if(A>0){const j=s.x,N=s.y-s.r+10;L+=`<circle cx="${j}" cy="${N}" r="7" fill="var(--info)" stroke="var(--surface)" stroke-width="1" data-for="${a(s.path)}" data-role="d-circle" />`,L+=`<text x="${j}" y="${N+3}" text-anchor="middle" fill="white" font-size="7" font-weight="700" style="pointer-events:none" data-for="${a(s.path)}" data-role="d-text">D</text>`}}T.innerHTML=L;let ot=!1,it=!1,F=null,st={vbX:0,vbY:0},w={x:C,y:Y,w:J,h:X};const pn={x:C,y:Y,w:J,h:X},le=4,gn=document.getElementById("metrics-hover-panel");function Ht(){T.setAttribute("viewBox",`${w.x} ${w.y} ${w.w} ${w.h}`)}function de(s){const l=w.x+w.w/2,p=w.y+w.h/2;w.w*=s,w.h*=s,w.x=l-w.w/2,w.y=p-w.h/2,Ht()}function ce(s,l){const p=T.createSVGPoint();p.x=s,p.y=l;const y=T.getScreenCTM();if(y){const f=p.matrixTransform(y.inverse());return{x:f.x,y:f.y}}const g=T.getBoundingClientRect();return{x:w.x+(s-g.left)/g.width*w.w,y:w.y+(l-g.top)/g.height*w.h}}function me(){T.querySelectorAll(".dep-lines line").forEach(l=>l.remove());const s=T.querySelector(".dep-lines");if(s){const l=new Set;for(const p of Object.values(K))if(!(p.length<2))for(let y=0;y<p.length;y++)for(let g=y+1;g<p.length;g++){const f=[p[y],p[g]].sort().join("|");if(l.has(f))continue;l.add(f);const x=c.find(at=>at.path===p[y]),v=c.find(at=>at.path===p[g]);if(!x||!v)continue;const _=v.x-x.x,A=v.y-x.y,j=Math.sqrt(_*_+A*A)||1,N=_/j,ft=A/j,P=document.createElementNS("http://www.w3.org/2000/svg","line");P.setAttribute("x1",String(x.x+N*(x.r+2))),P.setAttribute("y1",String(x.y+ft*(x.r+2))),P.setAttribute("x2",String(v.x-N*(v.r+2))),P.setAttribute("y2",String(v.y-ft*(v.r+2))),P.setAttribute("stroke","var(--info)"),P.setAttribute("stroke-width","1.5"),P.setAttribute("stroke-opacity","0.4"),P.setAttribute("marker-end","url(#dep-arrow)"),s.appendChild(P)}}T.querySelectorAll("[data-drill]").forEach(l=>{const p=l.getAttribute("data-drill"),y=c.find(g=>g.path===p);y&&(l.setAttribute("cx",String(y.x)),l.setAttribute("cy",String(y.y)))}),T.querySelectorAll("[data-for]").forEach(l=>{const p=l.getAttribute("data-for"),y=l.getAttribute("data-role"),g=c.find(f=>f.path===p);g&&(y==="label"?(l.setAttribute("x",String(g.x)),l.setAttribute("y",String(g.y+2))):y==="t-circle"?(l.setAttribute("cx",String(g.x)),l.setAttribute("cy",String(g.y+g.r-10))):y==="t-text"?(l.setAttribute("x",String(g.x)),l.setAttribute("y",String(g.y+g.r-7))):y==="d-circle"?(l.setAttribute("cx",String(g.x)),l.setAttribute("cy",String(g.y-g.r+10))):y==="d-text"&&(l.setAttribute("x",String(g.x)),l.setAttribute("y",String(g.y-g.r+13))))})}function bn(s){const l=s.maintainability??0,y=`hsl(${Math.min(120,Math.max(0,l*1.2))}, 80%, 45%)`;gn.innerHTML=`
    ...
    `}T.querySelectorAll("[data-drill]").forEach(s=>{s.addEventListener("mouseenter",()=>{const l=s.getAttribute("data-drill"),p=c.find(y=>y.path===l);p&&bn(p)})});let G=null,rt={x:0,y:0},qt={x:0,y:0};T.addEventListener("mousedown",s=>{s.button===0&&(ot=!1,F=null,G=s.target,rt={x:s.clientX,y:s.clientY},it=!0,st={x:s.clientX,y:s.clientY,vbX:w.x,vbY:w.y})}),window.addEventListener("mousemove",s=>{var g;if(!it&&!F)return;const l=s.clientX-rt.x,p=s.clientY-rt.y;if(Math.abs(l)>=le||Math.abs(p)>=le){if(!ot){ot=!0;const f=(g=G==null?void 0:G.getAttribute)==null?void 0:g.call(G,"data-drill");if(f){const x=c.find(v=>v.path===f);if(x){F=x,it=!1,T.style.cursor="move";const v=ce(rt.x,rt.y);qt={x:x.x-v.x,y:x.y-v.y}}}F||(T.style.cursor="grabbing")}if(F){const f=ce(s.clientX,s.clientY);F.x=f.x+qt.x,F.y=f.y+qt.y,me()}else if(it){const f=T.getScreenCTM();if(f)w.x=st.vbX-l/f.a,w.y=st.vbY-p/f.d;else{const x=T.getBoundingClientRect();w.x=st.vbX-l/x.width*w.w,w.y=st.vbY-p/x.height*w.h}Ht()}}}),window.addEventListener("mouseup",s=>{var y;const l=ot,p=G;if(!l&&p){const g=(y=p.getAttribute)==null?void 0:y.call(p,"data-drill");g&&Ut(g)}F=null,it=!1,ot=!1,G=null,T.style.cursor="grab"}),(pe=document.getElementById("metrics-zoom-in"))==null||pe.addEventListener("click",()=>de(.7)),(ge=document.getElementById("metrics-zoom-out"))==null||ge.addEventListener("click",()=>de(1.4)),(be=document.getElementById("metrics-zoom-fit"))==null||be.addEventListener("click",()=>{w={...pn},Ht()});const Pt=document.createElement("div");Pt.style.cssText="position:absolute;bottom:8px;left:8px;background:var(--bg);border:1px solid var(--border);border-radius:4px;padding:6px 10px;font-size:11px;line-height:1.6;opacity:0.9;z-index:2",Pt.innerHTML=`
  ...
  `,(ye=(he=e.querySelector("div > div:first-child"))==null?void 0:he.parentElement)==null||ye.appendChild(Pt);const hn=25;let yt=0;function Ot(){if(F){yt=requestAnimationFrame(Ot);return}for(let s=0;s<c.length;s++)for(let l=s+1;l<c.length;l++){const p=c[l].x-c[s].x,y=c[l].y-c[s].y,g=Math.sqrt(p*p+y*y)||.1,f=c[s].r+c[l].r+hn,x=p/g,v=y/g;if(g<f){const _=(f-g)*.15;c[s].x-=x*_,c[s].y-=v*_,c[l].x+=x*_,c[l].y+=v*_}else{const _=(g-f)*.008;c[s].x+=x*_,c[s].y+=v*_,c[l].x-=x*_,c[l].y-=v*_}}me(),yt=requestAnimationFrame(Ot)}yt=requestAnimationFrame(Ot),new MutationObserver(()=>{document.getElementById("metrics-svg")||cancelAnimationFrame(yt)}).observe(e,{childList:!0})}async function Ut(t){const e=document.getElementById("metrics-detail");if(!e)return;e.innerHTML='<p class="text-muted">Loading file analysis...</p>';const n=await B("/metrics/file?path="+encodeURIComponent(t));if(n.error){e.innerHTML=`<p style="color:var(--danger)">${a(n.error)}</p>`;return}const i=n.functions||[],o=n.warnings||[];e.innerHTML=`
  ...
  `}function E(t,e){return`<div class="metric-card"><div class="label">${a(t)}</div><div class="value">${a(String(e??0))}</div></div>`}window.__loadQuickMetrics=Ne,window.__loadFullMetrics=Wt,window.__drillDown=Ut;const dt={tina4:{model:"tina4-v1",url:"https://api.tina4.com/v1/chat/completions"},custom:{model:"",url:"http://localhost:11434"},anthropic:{model:"claude-sonnet-4-20250514",url:"https://api.anthropic.com"},openai:{model:"gpt-4o",url:"https://api.openai.com"}};function ct(t="tina4"){const e=dt[t]||dt.tina4;return{provider:t,model:e.model,url:e.url,apiKey:""}}function $t(t){const e={...ct(),...t||{}};return e.provider==="ollama"&&(e.provider="custom"),e}function Fe(){try{const t=JSON.parse(localStorage.getItem("tina4_chat_settings")||"{}");return{thinking:$t(t.thinking),vision:$t(t.vision),imageGen:$t(t.imageGen)}}catch{return{thinking:ct(),vision:ct(),imageGen:ct()}}}function Ge(t){localStorage.setItem("tina4_chat_settings",JSON.stringify(t)),I=t,V()}let I=Fe(),q="Idle";const mt=[];function We(t){var n,i,o,r,m,d,u,b,c,$;t.innerHTML=`
    <div class="dev-panel-header">
      <h2>Code With Me</h2>
      <div class="flex gap-sm">
...
`,i.focus(),i.setSelectionRange(i.value.length,i.value.length))}function Qe(t){var i,o;const e=t.closest(".chat-checklist-item");if(!e||(i=e.nextElementSibling)!=null&&i.classList.contains("chat-comment-box"))return;const n=document.createElement("div");n.className="chat-comment-box",n.style.cssText="padding-left:1.8rem;margin:0.15rem 0;display:flex;gap:4px",n.innerHTML=`
  ...
  `,e.after(n),(o=n.querySelector("input"))==null||o.focus()}function Ze(t){var r;const e=t.closest(".chat-comment-box");if(!e)return;const n=e.querySelector("input"),i=(r=n==null?void 0:n.value)==null?void 0:r.trim();if(!i)return;const o=document.createElement("div");o.style.cssText="padding-left:1.8rem;margin:0.1rem 0;font-size:0.7rem;color:var(--info);font-style:italic",o.textContent=`↳ ${i}`,e.replaceWith(o)}function Vt(){const t=[],e=[],n=[];return document.querySelectorAll(".chat-checklist-item").forEach(i=>{var d,u;const o=i.querySelector("input[type=checkbox]"),r=((d=i.querySelector("label"))==null?void 0:d.textContent)||"";o!=null&&o.checked?t.push(r):e.push(r);const m=i.nextElementSibling;if(m&&!m.classList.contains("chat-checklist-item")&&!m.classList.contains("chat-comment-box")){const b=((u=m.textContent)==null?void 0:u.replace("↳ ",""))||"";b&&n.push(`${r}: ${b}`)}}),{accepted:t,rejected:e,comments:n}}let ut=!1;function Lt(){const t=document.getElementById("chat-thoughts-panel");t&&(ut=!ut,t.style.display=ut?"block":"none",ut&&Yt())}async function Yt(){const t=document.getElementById("thoughts-list");if(t)try{const i=(await(await fetch("/__dev/api/thoughts")).json()||[]).filter(r=>!r.dismissed),o=document.getElementById("thoughts-dot");if(o&&(o.style.display=i.length?"inline":"none"),!i.length){t.innerHTML='<div class="text-muted text-sm" style="text-align:center;padding:2rem 0">All clear. No observations.</div>';return}t.innerHTML=i.map(r=>`
    ...
    `).join("")}catch{t.innerHTML='<div class="text-muted text-sm" style="text-align:center;padding:1rem">Agent not connected</div>'}}async function Jt(t){await fetch("/__dev/api/thoughts/dismiss",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:t})}).catch(()=>{}),Yt()}function tn(t,e){Jt(t),Lt()}setInterval(async()=>{try{const n=(await(await fetch("/__dev/api/thoughts")).json()||[]).filter(o=>!o.dismissed),i=document.getElementById("thoughts-dot");i&&(i.style.display=n.length?"inline":"none")}catch{}},6e4),window.__dismissThought=Jt,window.__actOnThought=tn,window.__commentOnItem=Qe,window.__submitComment=Ze,window.__getChecklist=Vt,window.__copyMsg=Xe,window.__replyMsg=Ke;const Q=[];function Xt(t){const e=document.getElementById("chat-status-bar"),n=document.getElementById("chat-status-text");e&&(e.style.display="flex"),n&&(n.textContent=t)}function Kt(){const t=document.getElementById("chat-status-bar");t&&(t.style.display="none")}function pt(t,e){const n=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"});Q.unshift({time:n,text:t,agent:e}),Q.length>50&&(Q.length=50),V()}async function Z(){var i;const t=document.getElementById("chat-input"),e=(i=t==null?void 0:t.value)==null?void 0:i.trim();if(!e)return;if(t.value="",z(a(e),"user"),D.length){const o=D.map(r=>r.name).join(", ");z(`<span class="text-sm text-muted">Attached: ${a(o)}</span>`,"user")}q="Thinking...",Xt("Analyzing request..."),pt("Analyzing request...","supervisor");const n={message:e,settings:{thinking:I.thinking,vision:I.vision,imageGen:I.imageGen}};D.length&&(n.files=D.map(o=>({name:o.name,data:o.data})));try{const o=await fetch("/__dev/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!o.ok||!o.body){z(`<span style="color:var(--danger)">Error: ${o.statusText}</span>`,"bot"),q="Error",V();return}const r=o.body.getReader(),m=new TextDecoder;let d="";for(;;){const{done:u,value:b}=await r.read();if(u)break;d+=m.decode(b,{stream:!0});const c=d.split(`
`);d=c.pop()||"";let $="";for(const h of c)if(h.startsWith("event: "))$=h.slice(7).trim();else if(h.startsWith("data: ")){const k=h.slice(6);try{const S=JSON.parse(k);Qt($,S)}catch{}}}D.length=0,Ct()}catch{z('<span style="color:var(--danger)">Connection failed</span>',"bot"),q="Error",V()}}function Qt(t,e){switch(t){case"status":q=e.text||"Working...",Xt(`${e.agent||"supervisor"}: ${e.text||"Working..."}`),pt(e.text||"",e.agent||"supervisor");break;case"message":{const n=e.content||"",i=e.agent||"supervisor";let o=dn(n);i!=="supervisor"&&(o=`<span class="badge" style="font-size:0.6rem;margin-right:4px">${a(i)}</span>`+o),e.files_changed&&e.files_changed.length>0&&(o+='<div style="margin-top:0.5rem;padding:0.5rem;background:var(--bg);border-radius:0.375rem;border:1px solid var(--border)">',o+='<div class="text-sm" style="color:var(--success);font-weight:600;margin-bottom:0.25rem">Files changed:</div>',e.files_changed.forEach(r=>{o+=`<div class="text-sm text-mono">${a(r)}</div>`,mt.includes(r)||mt.push(r)}),o+="</div>"),z(o,"bot");break}case"plan":if(e.approve){const n=`
...
`),o=[];for(const m of i){const d=m.trim();if(d.startsWith("\0CODE")){o.push(d);continue}if(d.startsWith("### ")){o.push(`<div style="font-weight:700;font-size:0.8rem;margin:0.75rem 0 0.25rem;color:var(--info)">${d.slice(4)}</div>`);continue}if(d.startsWith("## ")){o.push(`<div style="font-weight:700;font-size:0.9rem;margin:0.75rem 0 0.25rem">${d.slice(3)}</div>`);continue}if(d.startsWith("# ")){o.push(`<div style="font-weight:700;font-size:1rem;margin:0.75rem 0 0.25rem">${d.slice(2)}</div>`);continue}if(d==="---"||d==="***"){o.push('<hr style="border:none;border-top:1px solid var(--border);margin:0.5rem 0">');continue}const u=d.match(/^(\d+)[.)]\s+(.+)/);if(u){if(u[2].trim().endsWith("?")){const c=`q-${Mt}-${u[1]}`;o.push(`<div style="margin:0.3rem 0;padding-left:0.5rem">
  ...
  `;const e=document.getElementById("tab-bar");e.innerHTML=bt.map(n=>`<button class="dev-tab ${n.id===nt?"active":""}" data-tab="${n.id}" onclick="window.__switchTab('${n.id}')">${n.label}</button>`).join(""),zt(nt)}function zt(t){nt=t,document.querySelectorAll(".dev-tab").forEach(o=>{o.classList.toggle("active",o.dataset.tab===t)});const e=document.getElementById("tab-content");if(!e)return;const n=document.createElement("div");n.className="dev-panel active",e.innerHTML="",e.appendChild(n);const i=bt.find(o=>o.id===t);i&&i.render(n)}function un(){if(window.parent!==window)try{const t=window.parent.document.getElementById("tina4-dev-panel");t&&t.remove()}catch{document.body.style.display="none"}}window.__closeDevAdmin=un,window.__switchTab=zt,mn();let At=0,jt=null;(ne=document.getElementById("version-label"))==null||ne.addEventListener("click",()=>{if(!gt&&(At++,jt&&clearTimeout(jt),jt=setTimeout(()=>{At=0},2e3),At>=5)){gt=!0,localStorage.setItem("tina4_cwm_unlocked","true"),bt=[ee,...Bt],nt="chat";const t=document.getElementById("tab-bar");t&&(t.innerHTML=bt.map(e=>`<button class="dev-tab ${e.id===nt?"active":""}" data-tab="${e.id}" onclick="window.__switchTab('${e.id}')">${e.label}</button>`).join("")),zt("chat")}})})();
