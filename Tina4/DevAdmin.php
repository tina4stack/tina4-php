...
class DevAdmin
{
    ...
    public static function register(): void
    {
        ...
        // Dev-toolbar assets, served as external CSS/JS so the injected
        // toolbar is CSP-clean (no inline styles, handlers, or <script>) and
        // works under a strict `default-src 'self'`. text()+header() rather
        // than html() so the toolbar is not injected into its own assets
        // (see the SPA-bundle note above).
        Router::get('/__dev/toolbar.css', function (Request $request, Response $response) {
            return $response->text(self::toolbarCss())->header('Content-Type', 'text/css; charset=UTF-8');
        });
        Router::get('/__dev/toolbar.js', function (Request $request, Response $response) {
            return $response->text(self::toolbarJs())->header('Content-Type', 'application/javascript; charset=UTF-8');
        });
        ...
        // and the toolbar's own "Could not check for updates" branch could
        // never fire because the failure arrived as a success. latest is null
        // when the check could not be made, and error says why.
        //
        // The registry URL is TINA4_VERSION_CHECK_URL when set (a mirror, or a
        // test's own server), else Packagist — the same seam either way.
        Router::get('/__dev/api/version-check', function (Request $request, Response $response) {
            $current = App::$VERSION;
            $url = getenv('TINA4_VERSION_CHECK_URL')
                ?: ($_ENV['TINA4_VERSION_CHECK_URL'] ?? null)
                ?: 'https://repo.packagist.org/p2/tina4stack/tina4php.json';
        ...
        Router::get('/__dev/api/table', function (Request $request, Response $response) {
            ...
            try {
                ...
                // ADR-0078: only a name the database itself reports is accepted,
        ...
        Router::get('/__dev/api/file', function (Request $request, Response $response) {
            ...
            // judged on the RESOLVED path so `.env/.` or a symlink cannot slip past (ADR-0078).
        ...
        Router::get('/__dev/api/file/raw', function (Request $request, Response $response) {
            ...
            // DEVADMIN-DEC-03: never serve secret material, judged on the RESOLVED path (ADR-0078).
    ...
    private static function devAdminSafePath(string $input): ?string
    {
        ...
        // ADR-0078: follow symlinks the way the OS will when it opens the
        ...
        if ($real === false) {
            return is_link($existing) ? null : $absolute;
        }
     ...
     * Compared timing-safe against TINA4_MCP_TOKEN only; the application's
     * TINA4_API_KEY never unlocks the dev surface (ADR-0078). With NO configured
     * token this returns false, so a
     ...
     * TINA4_HOST (ADR-0078). A rebound DNS name is refused. A missing Host is
     ...
     * null to allow (ADR-0078): Host allow-list, then same-origin, then the
    ...
    public static function guardRequest(Request $request): ?array
    {
        ...
        if (!self::devHostAllowed($request->headers ?? []) && !self::mcpTokenOk($request)) {
...
<div id="tina4-dev-toolbar" data-reload="{$reload}">
    <span id="tina4-ver-btn" title="Click to check for updates">Tina4 v{$version}</span>
    ...
    <span class="t4-green">{$safeMethod}</span>
    <span>{$safePath}</span>
    <span class="t4-arrow">&rarr; {$safePattern}</span>
    <span class="t4-yellow">req:{$safeRequestId}</span>
    <span class="t4-blue">{$routeCount} routes</span>
    <span class="t4-dim">PHP {$phpVersion}</span>
...
HTML;
     ...
     * under a strict `default-src 'self'`.
     */
    private static function toolbarCss(): string
    {
        return <<<'CSS'
...
#tina4-ver-btn{color:#7b1fa2;font-weight:bold;cursor:pointer;text-decoration:underline dotted}
#tina4-ver-modal{display:none;position:fixed;bottom:3rem;left:1rem;background:#1e1e2e;border:1px solid #7b1fa2;border-radius:8px;padding:16px 20px;z-index:100000;min-width:320px;box-shadow:0 8px 32px rgba(0,0,0,.5);font-family:monospace;font-size:13px;color:#cdd6f4}
...
#tina4-dev-panel iframe{width:100%;height:100%;border:1px solid #7b1fa2;border-radius:.5rem;box-shadow:0 8px 32px rgba(0,0,0,.5);background:#0f172a}
CSS;
    }
     ...
     * CSP-clean. The reloader only starts when the toolbar's `data-reload` is
     * "1" (reload not suppressed for this request).
     */
    private static function toolbarJs(): string
    {
        return <<<'JS'
(function () {
    ...
    function checkVersion() {
        ...
        fetch('/__dev/api/version-check').then(function (r) { return r.json(); }).then(function (d) {
            ...
            if (isNewer) {
                ...
                if (breaking) {
                    el.innerHTML += '<div class="t4-err t4-note">&#9888; Major/minor version change &mdash; check the <a href="https://github.com/tina4stack/tina4-php/releases" target="_blank" class="t4-link">changelog</a> for breaking changes before upgrading.</div>';
                } else {
                    el.innerHTML += '<div class="t4-warn t4-note">Patch update available. Run: <code class="t4-code">composer update tina4stack/tina4-php</code></div>';
                }
            } else if (isAhead) {
                el.className = 't4-purple';
                el.innerHTML = 'You are running <strong class="t4-purple">v' + current + '</strong> (ahead of Packagist <strong>v' + latest + '</strong> &mdash; not yet published).';
            } else {
                upToDate(el, latest);
            }
        }).catch(function () {
