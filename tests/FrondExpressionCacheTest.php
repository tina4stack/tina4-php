...
class FrondExpressionCacheTest extends TestCase
{
    ...
    public static function cachedExpressionProvider(): array
    {
        return [
            ...
            // -- 'or' / 'and' / 'not' ----------------------------------------
            'and false'               => ['flagTrue and flagFalse', 'false'],
            'or true'                 => ['flagTrue or flagFalse', 'true'],
            'not true'                => ['not flagTrue', 'false'],
            'not false'               => ['not flagFalse', 'true'],
            'and of comparisons'      => ['n > m and m > 0', 'true'],
            'or of comparison'        => ['n < m or flagTrue', 'true'],

            // -- 'in' / 'notIn' ----------------------------------------------
            'in hit'                  => ['"a" in items', 'true'],
            'in miss'                 => ['"q" in items', 'false'],
            'not in hit'              => ['"a" not in items', 'false'],
            'not in miss'             => ['"q" not in items', 'true'],
            'in nested list'          => ['"admin" in user.roles', 'true'],

            // -- 'isTest' / 'isNotTest' (isKnownTest stays live) -------------
            'is custom test hit'      => ['word is spicy', 'true'],
            'is custom test miss'     => ['name is spicy', 'false'],
            'is not custom test'      => ['word is not spicy', 'false'],
            'is null'                 => ['nothing is null', 'true'],
            'is not null'             => ['nothing is not null', 'false'],
            'is defined'              => ['name is defined', 'true'],
            'is defined miss'         => ['missingKey is defined', 'false'],
            'is iterable'             => ['nums is iterable', 'true'],
            'is odd'                  => ['one is odd', 'true'],
            'is even miss'            => ['n is even', 'false'],
            ...
            'greater'                 => ['n > m', 'true'],
            'less'                    => ['n < m', 'false'],
            'greater or equal'        => ['n >= 7', 'true'],
            'less or equal miss'      => ['n <= 6', 'false'],
            'string equal'            => ['name == "Alice"', 'true'],
            'string not equal'        => ['name != "Bob"', 'true'],
            'apostrophe in string'    => ['quoted == "it\'s fine"', 'true'],
            'operator inside string'  => ['"a>b" == "a>b"', 'true'],
            'equals inside string'    => ['"x==y" != "z"', 'true'],
    ...
    public function testDescriptorBakesInNoContextValue(): void
    {
        $cases = [
            ...
            '{{ a > b }}'                      => [['a' => 5, 'b' => 1], ['a' => 1, 'b' => 5], 'true', 'false'],
            '{{ a|upper }}'                    => [['a' => 'one'], ['a' => 'two'], 'ONE', 'TWO'],
            '{{ a and b }}'                    => [['a' => true, 'b' => true], ['a' => true, 'b' => false], 'true', 'false'],
            '{{ a in b }}'                     => [['a' => 'k', 'b' => ['k']], ['a' => 'k', 'b' => ['z']], 'true', 'false'],
        ];
