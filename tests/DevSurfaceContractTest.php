 ...
 *
 * Dev-surface gate contract (ADR-0078) — REAL dispatch and a REAL server, no mocks.
 *
 * Drives tina4-documentation/plan/v3/fixtures/devsurface_contract.json through the
...
class DevSurfaceContractTest extends TestCase
{
    ...
    private const KEYS = [
        ...
        'TINA4_API_KEY', 'TINA4_HOST', 'TINA4_DATABASE_URL', 'TINA4_SECRET', 'SECRET',
    ];
    ...
    protected function setUp(): void
    {
        ...
        $this->origCwd = getcwd() ?: '.';
        $this->baseDir = sys_get_temp_dir() . '/tina4_surface_' . bin2hex(random_bytes(6));
        $this->projectDir = $this->baseDir . '/app';
        mkdir($this->projectDir . '/src/templates/pages', 0777, true);
        mkdir($this->projectDir . '/src/routes', 0777, true);
        mkdir($this->baseDir . '/app-sibling', 0777, true);
    ...
    public function testANonLoopbackPeerCannotRead(): void
    {
        DevAdmin::register();
        $resp = $this->dispatch('GET', '/__dev/api/file?path=readme.txt', [], '203.0.113.9');
        $this->assertSame(403, $resp->getStatusCode());
        $this->assertStringNotContainsString('public-readme', $resp->getBody());
    }
    ...
    public function testTheApiKeyDoesNotUnlockDevWrites(): void
    {
        ...
        $this->assertSame(404, $mcp->getStatusCode());
        $this->env('TINA4_MCP_TOKEN', 'mcp-token-0078');
        $ok = $this->dispatch('POST', '/__dev/api/file/save', ['authorization' => 'Bearer mcp-token-0078', 'host' => 'devbox.lan:7145'], '203.0.113.9',
    ...
    // ── Health does not disclose the version in production ───────

    public function testHealthOmitsTheVersionOutsideDebug(): void
    {
        ...
        foreach (['/health', '/__health'] as $path) {
            ...
            sort($keys);
            $this->assertSame(['framework', 'status', 'uptime'], $keys);
        }
